...
#define DEBUG_TYPE "Koda-frame-lowering"

using namespace llvm;

void KodaFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                             BitVector &SavedRegs,
...
// TODO: Build insns
void KodaFrameLowering::adjustReg(MachineBasicBlock &MBB,
                                  MachineBasicBlock::iterator MBBI,
                                  const DebugLoc &DL, Register DestReg,
                                  Register SrcReg, int64_t Val,
                                  MachineInstr::MIFlag Flag) const {
  // MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  const KodaInstrInfo *TII = STI.getInstrInfo();

  if (DestReg == SrcReg && Val == 0)
    return;

  if (isInt<12>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(Koda::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
...
void KodaFrameLowering::adjustStackToMatchRecords(
...
void KodaFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  auto *FI = MF.getInfo<KodaFunctionInfo>();
  ...
  if (!isInt<12>(StackSize)) {
    llvm_unreachable("Stack offs won't fit in load insn");
  }
...
// TODO: seems ok
void KodaFrameLowering::emitEpilogue(MachineFunction &MF,
                                     MachineBasicBlock &MBB) const {
  const KodaRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
// TODO: seems ok
bool KodaFrameLowering::spillCalleeSavedRegisters(
...
// TODO: seems ok
bool KodaFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
...
// TODO: seems ok
void KodaFrameLowering::processFunctionBeforeFrameFinalized(
...
MachineBasicBlock::iterator KodaFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == Koda::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
bool KodaFrameLowering::hasFP(const MachineFunction &MF) const {
...
// TODO: rewrite!
StackOffset
KodaFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
                                          Register &FrameReg) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo *RI = MF.getSubtarget().getRegisterInfo();
  const auto *UFI = MF.getInfo<KodaFunctionInfo>();
...
bool KodaFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
