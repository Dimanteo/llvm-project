...
KodaTargetLowering::KodaTargetLowering(const TargetMachine &TM,
                                       const KodaSubtarget &STI)
    : TargetLowering(TM), STI(STI) {
  addRegisterClass(MVT::i32, &Koda::HartRegsRegClass);

  computeRegisterProperties(STI.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(Koda::X2);
...
const char *KodaTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case KodaISD::CALL:
    return "KodaISD::CALL";
...
#include "KodaGenCallingConv.inc"
...
/// Koda formal arguments implementation
// TODO: rewrite
SDValue KodaTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_Koda);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &Koda::HartRegsRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    KodaFunctionInfo *UFI = MF.getInfo<KodaFunctionInfo>();
...
SDValue KodaTargetLowering::PerformDAGCombine(SDNode *N,
...
// TODO: verify
bool KodaTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
bool KodaTargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  return false;
}
...
SDValue KodaTargetLowering::lowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(KodaISD::BR_CC, DL, Op.getValueType(), Op.getOperand(0),
                     LHS, RHS, TargetCC, Block);
}

SDValue KodaTargetLowering::lowerFRAMEADDR(SDValue Op,
                                           SelectionDAG &DAG) const {
...
SDValue KodaTargetLowering::LowerOperation(SDValue Op,
                                           SelectionDAG &DAG) const {
  switch (Op->getOpcode()) {
  case ISD::BR_CC:
